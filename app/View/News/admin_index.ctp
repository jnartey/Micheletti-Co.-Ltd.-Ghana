				...
				<tr>
					...
					<td class="actions">
						<?php 
							...
							if($data['News']['featured'] == 0){
								echo  $this->Form->postLink(__("Feature", true), array('controller'=>'news', 'action'=>'featured', $data['News']['id'], 'admin' => true), array('escape'=>false, 'class'=>'button tiny secondary'), sprintf(__('Are you sure you want to feature ~ '.$data['News']['title'], true), $data['News']['id']));
							}else{
								echo  $this->Form->postLink(__("Featured", true), array('controller'=>'news', 'action'=>'unfeature', $data['News']['id'], 'admin' => true), array('escape'=>false, 'class'=>'button tiny success'), sprintf(__('Are you sure you want to remove ~ '.$data['News']['title'].' from featured?', true), $data['News']['id']));
							}
							if($data['News']['archive'] == 0){
								...
								echo  $this->Form->postLink(__("Archived", true), array('controller'=>'news', 'action'=>'unarchive_news', $data['News']['id'], 'admin' => true), array('escape'=>false, 'class'=>'button tiny success'), sprintf(__('Are you sure you want to remove from archive ~ '.$data['News']['title'], true), $data['News']['id']));
							}
