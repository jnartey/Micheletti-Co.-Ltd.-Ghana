...
class PageContentsController extends AppController {
	...
	public function admin_index() {
		...
		$this->paginate = array('conditions'=>array('NOT'=>array('PageContent.title'=>''), 'OR'=>array('PageContent.category'=>null, )), 'order'=>array('PageContent.id DESC'), 'limit' => 15);
	...
	public function admin_add($id = null, $set = null) {
		...
		if ($this->request->is('post')) {
			if ($this->PageContent->save($this->request->data)) {
				...
				if($this->request->data['PageContent']['gallery'] == 1){
					$data = array('link_id' => $this->PageContent->getInsertID(), 'name' => $this->request->data['PageContent']['title']);
		...
		$categories = $this->PageContent->Category->generateTreeList(null, null, null, '--');
		
		$this->set(compact('header', 'current_page', 'page', 'page_parent', 'rev', 'gallery', 'parent', 'project_category', 'set', 'categories'));
	}
	...
	public function admin_edit($id = null, $set = null) {
		...
		if ($this->request->is('post') || $this->request->is('put')) {
			...
			if ($this->PageContent->save($this->request->data)) {
				if($this->request->data['PageContent']['gallery'] == 1){
					$data = array('link_id' => $page_parent[0]['PageContent']['id'], 'name' => $this->request->data['PageContent']['title']);
					$this->Gallery->save($data);
				}
		...
		$this->set(compact('header', 'current_page', 'page', 'page_parent', 'gallery', 'parent', 'project_category', 'parent_node', 'set', 'categories'));
	}
